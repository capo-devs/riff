...
namespace riff {
namespace imcpp {
auto InputText::update(klib::CString const name, ImVec2 const multi_size) -> bool {
	static constexpr auto flags_v = ImGuiInputTextFlags_CallbackResize;
	if (m_buffer.empty()) { m_buffer.resize(init_size_v, '\0'); }
	auto const cb = [](ImGuiInputTextCallbackData* data) {
		return static_cast<InputText*>(data->UserData)->on_callback(*data);
	};
	if (multi_size.x <= 0.0f || multi_size.y <= 0.0f) {
		return ImGui::InputText(name.c_str(), m_buffer.data(), m_buffer.size(), flags_v, cb, this);
	}
	return ImGui::InputTextMultiline(name.c_str(), m_buffer.data(), m_buffer.size(), {multi_size.x, multi_size.y},
									 flags_v, cb, this);
}
